...
#dwawting contours to detect shape
def detect_shape(f1):
    ...
    for cnt in contours:
        #detecting shape coordinates
        approx = cv2.approxPolyDP(cnt, 0.01*cv2.arcLength(cnt, True), True)
        
        #drawign shapes using those coordinates
        cv2.drawContours(img, [approx], 0, (0), 5)

        x = approx.ravel()[0] #all x coordintes stored here
        y = approx.ravel()[1] #all y coordintes stored here
